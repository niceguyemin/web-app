...
export default async function SettingsPage() {
    ...
    const users = isAdmin ? await prismadb.user.findMany({
        orderBy: { createdAt: "desc" },
    }) : [];

    const serviceTypes = isAdmin ? await prismadb.serviceType.findMany({
        orderBy: { createdAt: "desc" },
    }) : [];

    const logs = isAdmin ? await prismadb.log.findMany({
        take: 30,
        orderBy: { createdAt: "desc" },
        include: {
            user: {
    ...
    return (
        <div className="p-0 md:p-8 space-y-4 md:space-y-8">
            ...
            <Tabs defaultValue={isAdmin ? "users" : "profile"} className="space-y-4">
                <TabsList className="card border-0 p-1 mx-4 md:mx-0 h-auto flex-wrap justify-start">
                    {isAdmin && (
                        <>
                    ...
                    <TabsTrigger value="profile" className="data-[state=active]:bg-primary data-[state=active]:text-white">Profilim</TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="profile" className="space-y-4">
                    <Card className="card border-0 mx-4 md:mx-0">
                        <CardHeader>
                            <CardTitle className="text-text-heading">Profil Ayarları</CardTitle>
                            <CardDescription className="text-text-muted">Kişisel bilgilerinizi ve şifrenizi buradan güncelleyebilirsiniz.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <ProfileForm />
                        </CardContent>
